...
videojs.plugin('speed', function(options) {
    ...
    videojs.SpeedButton.prototype.createItems = function() {
        ...
        options.forEach(function(opt) {
            item = new videojs.SpeedMenuItem(player, {
                label: opt.text,
                rate: opt.rate
            });
            if (opt.selected) {
    ...
    player.ready(function() {
        var button = new videojs.SpeedButton(player, {
            el: videojs.Component.prototype.createEl(null, {
                ...
                innerHTML:
                    '<div class="vjs-control-content" style="font-size: 11px; line-height: 28px;"><span class="vjs-current-speed"></span></div>',
                role: 'button'
            })
        });
        ...
        try {
            if (player.c.P.id.indexOf('html5') == -1) {
                // FIXME player.c.P
                button.hide();
            }
        } catch (err) {
